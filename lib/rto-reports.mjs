    ...
    `
      with valid_reports as (
        ...
        left join rto_registration_observations previous_row
         ...
         and previous_row.request_hash = current_row.request_hash and previous_row.response_hash <> current_row.response_hash
         and previous_row.month_to_date_total <> current_row.month_to_date_total
      ...
      select c.state, c.rto,
        ...
        case when comparisons.current_scopes = 6 and comparisons.matched_scopes = 6 then comparisons.daily_ev else null end as daily_ev,
        case when comparisons.current_scopes = 6 and comparisons.matched_scopes = 6 then comparisons.daily_ice else null end as daily_ice,
        case when comparisons.current_scopes = 6 and comparisons.matched_scopes = 6 then comparisons.daily_total else null end as daily_total,
        jsonb_agg(
          jsonb_build_object(
  ...
  const currentCycleTrendResult = await query(
    `
      with cohort as (
        ...
        join rto_registration_observations previous_row
          on previous_row.state = current_row.state and previous_row.rto = current_row.rto
         ...
         and previous_row.response_hash <> current_row.response_hash
         and previous_row.month_to_date_total <> current_row.month_to_date_total
  ...
  const result = await query(
    `with cohort as (
       ...
       join rto_registration_observations p
        ...
        and p.response_hash <> c.response_hash
        and p.month_to_date_total <> c.month_to_date_total
     )
     select count(*)::int as count from (
       select c.state, c.rto
       from current_rows c

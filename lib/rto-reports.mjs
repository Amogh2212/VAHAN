...
export const RTO_REPORT_PAYLOAD_VERSION = 4;
export const RTO_REPORT_PDF_RENDERER_VERSION = 4;
...
function buildOemMetric({ index, member, oem, period, untracked = false }) {
  ...
  const categories = ["2W", "3W", "4W"].map((vehicleCategory) => {
    ...
    const stockTotal = sumOptional([stockEv, stockIce]);
  ...
  const stockEv = sumOptional(categories.map((row) => row.stock.ev));
  const stockIce = sumOptional(categories.map((row) => row.stock.ice));
  ...
  return {
    ...
    previousPeriod: { ev: previousEv, ice: previousIce, total: previousTotal },
    stock: { ev: stockEv, ice: stockIce, total: sumOptional([stockEv, stockIce]) },
    change: {
      ev: comparison(stockEv, previousEv),
      ice: comparison(stockIce, previousIce),
      total: comparison(sumOptional([stockEv, stockIce]), previousTotal),
    },
    categories,
  };
}

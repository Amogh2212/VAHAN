...
export async function getRtoReportReadiness({ runId = null } = {}) {
  ...
  const gateState = evaluateRtoReportReadinessGates({ run, cohortSize, completeRtos, comparisonEligibleRtos });
  ...
  return {
    ...
    oemContractRtos,
    gates: {
      collectionCompletion: { passed: collectionCompletion, succeededRtos: Number(run.succeeded_rtos ?? 0), totalRtos: Number(run.total_rtos ?? 0) },
...
export function evaluateRtoReportReadinessGates({ run, cohortSize = 0, completeRtos = 0, comparisonEligibleRtos = 0 } = {}) {
  const collectionCompletion = Boolean(run)
    ...
    && Number(run.total_rtos) > 0
    && Number(run.succeeded_rtos) === Number(run.total_rtos);
  let reason = null;

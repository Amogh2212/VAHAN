...
export const RTO_REPORT_PAYLOAD_VERSION = 5;
export const RTO_REPORT_PDF_RENDERER_VERSION = 5;
    ...
    return {
      ...
      payload: {
        ...
        source: {
          ...
          limitation: "These are active vehicle-stock observations. Net stock change includes corrections and status changes; it is not new registrations. Unchanged observations do not establish source freshness.",
...
const batchReadSql = `select b.*, stats.verified_report_count, stats.runtime_review_count, stats.runtime_warning_count
  ...
  cross join lateral (
    select count(*) filter (where payload->'source'->>'validationContract' = 'public-stock-v2')::int as verified_report_count,
      count(*) filter (where payload->'source'->>'validationContract' is distinct from 'public-stock-v2' or status = 'needs_review')::int as runtime_review_count,
      count(*) filter (where payload->'source'->>'validationContract' = 'public-stock-v2' and status = 'ready_with_warnings')::int as runtime_warning_count
...
export async function listRtoReportsForBatch(batchId, {
  ...
  if (status) {
    values.push(status);
    clauses.push(`(case when payload->'source'->>'validationContract' = 'public-stock-v2' then status else 'needs_review' end) = $${values.length}`);
  }
  values.push(boundedInt(limit, 1, 500, 100));
  ...
  const result = await query(
    `
      select
        ...
        payload->'source'->>'validationContract' as validation_contract
      from rto_reports

...
import { RTO_DAILY_METRIC_KIND, RTO_DAILY_STOCK_METRIC_KIND } from "./rto-daily-snapshots.mjs";

import { verifiedRtoRegistrationSourceSql } from "./public-dashboard-scope.mjs";
import {
  DAILY_REGISTRATION_REASON,
  DAILY_REGISTRATION_SOURCE_CONTRACT,
  buildDailyRegistrationEvidence,
  unavailableDailyRegistrationReport,
} from "./rto-daily-registration-contract.mjs";
...
export const RTO_REPORT_PDF_RENDERER_VERSION = 8;
...
export async function getRtoReportReadiness({ runId = null } = {}) {
  ...
  if (!run) {
    return {
      ...
      dailyRegistrationReason: "No frozen 100-RTO collection exists, so no consecutive monthly-registration comparison can be verified.",
      dailyRegistrationCoverage: 0,
      comparisonEligibleRtos: 0,
      previousDayEligibleRtos: 0,
      oemContractRtos: 0,
      gates: {
        collectionCompletion: { passed: false, succeededRtos: 0, totalRtos: 0 },
        cohortCoverage: { passed: false, actual: 0, expected: RTO_REPORT_EXPECTED_COHORT_SIZE },
        sixScopesPerRto: { passed: false, completeRtos: 0 },
        registrationSourceValidity: { passed: false, contract: DAILY_REGISTRATION_SOURCE_CONTRACT },
        oemSourceContractValidity: { passed: false, verifiedRtos: 0 },
        comparisonEligibility: { passed: false, eligibleRtos: 0 },
        dailyReportUsability: { passed: false, usableRtos: 0 },
      },
      missingRtos: [],
      run: null,
    };
    ...
    `
      ...
      valid_reports as (
        ...
        where r.run_id = $1
          ...
          and ${verifiedRtoRegistrationSourceSql("r")}
  ...
  const comparisonEligibleRtos = await registrationComparisonCoverage({ runId: run.id, date: dateOnly(run.snapshot_date), requireCurrentRun: true });
  const previousDayEligibleRtos = await registrationComparisonCoverage({ runId: run.id, date: addDays(dateOnly(run.snapshot_date), -1), requireCurrentRun: false });
  const oemResult = await query(
    `select count(*)::int as count from (
       select state, rto
       from rto_daily_scrape_reports r
       where run_id = $1 and metric_kind = '${RTO_DAILY_METRIC_KIND}'
         and status = 'success' and evidence->'oem'->>'status' = 'verified'
         and exists (select 1 from rto_daily_run_cohort_members c where c.run_id = $1 and c.state = r.state and c.rto = r.rto)
       group by state, rto having count(*) = 6
     ) verified_oem_rtos`,
    [run.id],
  );
  const oemContractRtos = Number(oemResult.rows[0]?.count ?? 0);
  const partialEvidenceResult = await query(
    `
      with valid_reports as (
        ...
        from valid_reports
        group by state, rto
        having count(distinct concat(fuel_group, chr(124), vehicle_category)) between 1 and 5
      )
      select r.state, r.rto, r.fuel_group, r.vehicle_category, r.report_total
      from valid_reports r
      join partial_rtos p using (state, rto)
      order by r.state, r.rto, r.fuel_group, r.vehicle_category
    `,
    [run.id],
  );
  const partialEvidence = new Map();
  for (const item of partialEvidenceResult.rows) {
    const key = `${item.state}||${item.rto}`;
    const entry = partialEvidence.get(key) ?? { state: item.state, rto: item.rto, scopes: [] };
    entry.scopes.push({ fuelGroup: item.fuel_group, vehicleCategory: item.vehicle_category, total: Number(item.report_total) });
    partialEvidence.set(key, entry);
  }
  const currentCycleEvidenceResult = await query(
    `
      with valid_reports as (
        select r.state, r.rto, r.fuel_group, r.vehicle_category, r.report_total
        from rto_daily_scrape_reports r
        ...
        select
          current_row.state,
          current_row.rto,
           count(*)::int as current_scopes,
           count(previous_row.id)::int as matched_scopes,
           count(*) filter (where current_row.fuel_group = 'EV'
             and current_row.month_to_date_total <> previous_row.month_to_date_total
             and current_row.response_hash <> previous_row.response_hash)::int as changed_ev_scopes,
           count(*) filter (where current_row.fuel_group = 'ICE'
             and current_row.month_to_date_total <> previous_row.month_to_date_total
             and current_row.response_hash <> previous_row.response_hash)::int as changed_ice_scopes,
           count(*) filter (where current_row.month_to_date_total <> previous_row.month_to_date_total
             and current_row.response_hash = previous_row.response_hash)::int as unverified_changed_scopes,
          sum(current_row.month_to_date_total - previous_row.month_to_date_total) filter (where current_row.fuel_group = 'EV')::int as daily_ev,
          sum(current_row.month_to_date_total - previous_row.month_to_date_total) filter (where current_row.fuel_group = 'ICE')::int as daily_ice,
          sum(current_row.month_to_date_total - previous_row.month_to_date_total)::int as daily_total
        from rto_registration_observations current_row
        join rto_daily_run_cohort_members cohort
          on cohort.run_id = $1 and cohort.state = current_row.state and cohort.rto = current_row.rto
        left join rto_registration_observations previous_row
          on previous_row.state = current_row.state and previous_row.rto = current_row.rto
         and previous_row.fuel_group = current_row.fuel_group and previous_row.vehicle_category = current_row.vehicle_category
         and previous_row.observation_date = current_row.observation_date - interval '1 day'
         and previous_row.accepted = true and previous_row.metric_kind = current_row.metric_kind
         and previous_row.target_month = current_row.target_month and previous_row.filters = current_row.filters
          and previous_row.request_hash = current_row.request_hash
        where current_row.run_id = $1 and current_row.observation_date = $2::date
          and current_row.accepted = true and current_row.metric_kind = '${RTO_DAILY_METRIC_KIND}'
        group by current_row.state, current_row.rto
      )
      select c.state, c.rto,
        count(distinct concat(r.fuel_group, chr(124), r.vehicle_category))::int as verified_scopes,
        coalesce(sum(r.report_total) filter (where r.fuel_group = 'EV'), 0)::int as ev_month_to_date,
        coalesce(sum(r.report_total) filter (where r.fuel_group = 'ICE'), 0)::int as ice_month_to_date,
        coalesce(sum(r.report_total), 0)::int as total_month_to_date,
         case when comparisons.current_scopes = 6 and comparisons.matched_scopes = 6
           and comparisons.changed_ev_scopes > 0 and comparisons.changed_ice_scopes > 0
           and comparisons.unverified_changed_scopes = 0
           and comparisons.daily_ev <> 0 and comparisons.daily_ice <> 0 and comparisons.daily_total <> 0
           then comparisons.daily_ev else null end as daily_ev,
         case when comparisons.current_scopes = 6 and comparisons.matched_scopes = 6
           and comparisons.changed_ev_scopes > 0 and comparisons.changed_ice_scopes > 0
           and comparisons.unverified_changed_scopes = 0
           and comparisons.daily_ev <> 0 and comparisons.daily_ice <> 0 and comparisons.daily_total <> 0
           then comparisons.daily_ice else null end as daily_ice,
         case when comparisons.current_scopes = 6 and comparisons.matched_scopes = 6
           and comparisons.changed_ev_scopes > 0 and comparisons.changed_ice_scopes > 0
           and comparisons.unverified_changed_scopes = 0
           and comparisons.daily_ev <> 0 and comparisons.daily_ice <> 0 and comparisons.daily_total <> 0
           then comparisons.daily_total else null end as daily_total,
         case
           when comparisons.current_scopes is distinct from 6 then 'Six accepted current-day registration scopes are required.'
           when comparisons.matched_scopes is distinct from 6 then 'Needs six compatible previous-day registration scopes.'
           when comparisons.unverified_changed_scopes > 0 then 'A registration total changed without a changed source response.'
           when comparisons.changed_ev_scopes = 0 or comparisons.changed_ice_scopes = 0
             or comparisons.daily_ev = 0 or comparisons.daily_ice = 0 or comparisons.daily_total = 0
             then 'An EV or ICE total is unchanged without a source refresh timestamp; zero Daily change is unverified.'
           else null
         end as daily_reason,
        jsonb_agg(
          jsonb_build_object(
            ...
            'total', r.report_total
          ) order by r.vehicle_category, r.fuel_group
        ) filter (where r.fuel_group is not null) as scopes
      from rto_daily_run_cohort_members c
      left join valid_reports r on r.state = c.state and r.rto = c.rto
      left join daily_comparisons comparisons on comparisons.state = c.state and comparisons.rto = c.rto
      where c.run_id = $1
       group by c.state, c.rto, comparisons.current_scopes, comparisons.matched_scopes,
         comparisons.changed_ev_scopes, comparisons.changed_ice_scopes, comparisons.unverified_changed_scopes,
         comparisons.daily_ev, comparisons.daily_ice, comparisons.daily_total
      order by c.state, c.rto
    `,
    [run.id, dateOnly(run.snapshot_date)],
  );
  const currentCycleTrendResult = await query(
    `
      with cohort as (
        select state, rto from rto_daily_run_cohort_members where run_id = $1
      ), daily_rows as (
        select current_row.state, current_row.rto, current_row.observation_date,
          current_row.fuel_group,
           current_row.month_to_date_total - previous_row.month_to_date_total as daily_total,
           previous_row.id as previous_id,
           current_row.response_hash <> previous_row.response_hash as response_changed
        from rto_registration_observations current_row
        join cohort on cohort.state = current_row.state and cohort.rto = current_row.rto
         left join rto_registration_observations previous_row
          on previous_row.state = current_row.state and previous_row.rto = current_row.rto
         and previous_row.fuel_group = current_row.fuel_group
         and previous_row.vehicle_category = current_row.vehicle_category
         and previous_row.observation_date = current_row.observation_date - interval '1 day'
         and previous_row.accepted = true and previous_row.metric_kind = current_row.metric_kind
         and previous_row.target_month = current_row.target_month and previous_row.filters = current_row.filters
         and previous_row.request_hash = current_row.request_hash
        where current_row.accepted = true
          and current_row.metric_kind = '${RTO_DAILY_METRIC_KIND}'
          and current_row.observation_date between ($2::date - interval '13 days') and $2::date
      )
      select state, rto, observation_date as date,
        sum(daily_total) filter (where fuel_group = 'EV')::int as ev,
        sum(daily_total) filter (where fuel_group = 'ICE')::int as ice,
        sum(daily_total)::int as total
       from daily_rows
       group by state, rto, observation_date
       having count(*) = 6 and count(previous_id) = 6
         and count(*) filter (where fuel_group = 'EV' and daily_total <> 0 and response_changed) > 0
         and count(*) filter (where fuel_group = 'ICE' and daily_total <> 0 and response_changed) > 0
         and count(*) filter (where daily_total <> 0 and response_changed = false) = 0
         and sum(daily_total) filter (where fuel_group = 'EV') <> 0
         and sum(daily_total) filter (where fuel_group = 'ICE') <> 0
         and sum(daily_total) <> 0
      order by state, rto, observation_date
    `,
    [run.id, dateOnly(run.snapshot_date)],
  );
  const currentCycleTrends = new Map();
  for (const row of currentCycleTrendResult.rows) {
    const key = `${row.state}||${row.rto}`;
    const trend = currentCycleTrends.get(key) ?? [];
    trend.push({ date: dateOnly(row.date), ev: row.ev === null ? null : Number(row.ev), ice: row.ice === null ? null : Number(row.ice), total: row.total === null ? null : Number(row.total) });
    currentCycleTrends.set(key, trend);
  }
  const fixedJobsResult = await query(`select count(*)::int as total,
    count(*) filter (where j.status = 'success')::int as succeeded
    from rto_daily_jobs j join rto_daily_run_cohort_members c
      on c.run_id = j.run_id and c.config_id = j.config_id
    where j.run_id = $1`, [run.id]);
  const fixedJobs = fixedJobsResult.rows[0];
  const gateState = evaluateRtoReportReadinessGates({ run: {
    ...run,
    fixed_total_rtos: Number(fixedJobs.total),
    fixed_succeeded_rtos: Number(fixedJobs.succeeded),
  }, cohortSize, completeRtos, comparisonEligibleRtos });
  const { reason, collectionCompletion, collectionEligible, comparisonEligible, dailyRegistrationEligible } = gateState;
  return {
    eligible: collectionEligible,
    collectionEligible,
    dailyRegistrationEligible,
    dailyRegistrationReason: dailyRegistrationEligible ? null : !collectionEligible
      ? `Collection or source validity is ${reason}; Daily registration reports are not usable even if comparison rows exist.`
      : `Only ${comparisonEligibleRtos} of ${RTO_REPORT_EXPECTED_COHORT_SIZE} RTOs have six compatible, consecutive, freshness-distinct monthly-registration observations.`,
    dailyRegistrationCoverage: comparisonEligibleRtos,
    comparisonEligibleRtos,
    previousDayEligibleRtos,
    oemContractRtos,
    gates: {
      collectionCompletion: { passed: collectionCompletion, succeededRtos: Number(fixedJobs.succeeded), totalRtos: Number(fixedJobs.total) },
      cohortCoverage: { passed: cohortSize === RTO_REPORT_EXPECTED_COHORT_SIZE, actual: cohortSize, expected: RTO_REPORT_EXPECTED_COHORT_SIZE },
      sixScopesPerRto: { passed: completeRtos === RTO_REPORT_EXPECTED_COHORT_SIZE, completeRtos },
      registrationSourceValidity: { passed: completeRtos === RTO_REPORT_EXPECTED_COHORT_SIZE, contract: DAILY_REGISTRATION_SOURCE_CONTRACT },
      oemSourceContractValidity: { passed: oemContractRtos === RTO_REPORT_EXPECTED_COHORT_SIZE, verifiedRtos: oemContractRtos },
      comparisonEligibility: { passed: comparisonEligible, eligibleRtos: comparisonEligibleRtos },
      dailyReportUsability: { passed: dailyRegistrationEligible, usableRtos: dailyRegistrationEligible ? comparisonEligibleRtos : 0 },
    },
    reason,
    expectedRtos: RTO_REPORT_EXPECTED_COHORT_SIZE,
    expectedOems: RTO_REPORT_EXPECTED_OEMS,
    ...
    partialEvidence: [...partialEvidence.values()],
    currentCycleEvidence: currentCycleEvidenceResult.rows.map((item) => ({
      state: item.state,
      rto: item.rto,
      verifiedScopes: Number(item.verified_scopes),
      evMonthToDate: Number(item.ev_month_to_date),
      iceMonthToDate: Number(item.ice_month_to_date),
      totalMonthToDate: Number(item.total_month_to_date),
      daily: {
        ev: finiteOrNull(item.daily_ev),
        ice: finiteOrNull(item.daily_ice),
        total: finiteOrNull(item.daily_total),
         status: item.daily_total !== null && Number.isFinite(Number(item.daily_total))
           ? Number(item.daily_total) < 0 ? "correction" : "available"
           : "unavailable",
         reason: item.daily_reason,
      },
      scopes: Array.isArray(item.scopes) ? item.scopes.map((scope) => ({
        fuelGroup: scope.fuelGroup,
        vehicleCategory: scope.vehicleCategory,
        total: Number(scope.total),
      })) : [],
      trend: currentCycleTrends.get(`${item.state}||${item.rto}`) ?? [],
    })),
    run: normalizeRun(rowToCamel(run)),
  };
}

export function evaluateRtoReportReadinessGates({ run, cohortSize = 0, completeRtos = 0, comparisonEligibleRtos = 0 } = {}) {
  const collectionCompletion = Boolean(run)
    && Number(run.fixed_total_rtos ?? run.total_rtos) > 0
    && Number(run.fixed_succeeded_rtos ?? run.succeeded_rtos) === Number(run.fixed_total_rtos ?? run.total_rtos);
  let reason = null;
  if (!collectionCompletion) reason = "collection_incomplete";
  else if (Number(cohortSize) !== RTO_REPORT_EXPECTED_COHORT_SIZE) reason = "cohort_size_not_100";
  else if (Number(completeRtos) !== RTO_REPORT_EXPECTED_COHORT_SIZE) reason = "cohort_incomplete";
  const collectionEligible = reason === null;
  const comparisonEligible = Number(comparisonEligibleRtos) === RTO_REPORT_EXPECTED_COHORT_SIZE;
  return {
    reason,
    collectionCompletion,
    collectionEligible,
    comparisonEligible,
    dailyRegistrationEligible: collectionEligible && comparisonEligible,
  };
}
...
export async function materializeRtoReportFactsForRun({
  ...
  if (!Number(cohortCount.rows[0]?.count) && !monitored) {
  ...
  const scopeCte = monitored
    ? `select j.state, j.rto from rto_daily_jobs j where j.run_id = $1
       and not exists (select 1 from rto_daily_run_cohort_members c
         where c.run_id = j.run_id and c.config_id = j.config_id)`
    : "select state, rto from rto_daily_run_cohort_members where run_id = $1";
...
export function buildRtoReportPayloads({
  ...
  if (period.cadence === "daily") {
    return buildDailyRegistrationReports({ period, cohort: normalizedCohort, totals, oems, generatedAt });
  }
  ...
  const reports = normalizedCohort.map((member) => {
    ...
    return {
      ...
      payload: {
        ...
        metricKind: RTO_DAILY_STOCK_METRIC_KIND,
...
async function registrationComparisonCoverage({ runId, date, requireCurrentRun }) {
  if (!date) return 0;
  const result = await query(
    `with cohort as (
       select state, rto from rto_daily_run_cohort_members where run_id = $1
     ), current_rows as (
       select o.* from rto_registration_observations o
       join cohort c using (state, rto)
       where o.observation_date = $2::date and o.accepted = true
         and o.metric_kind = '${RTO_DAILY_METRIC_KIND}'
         and ($3::boolean = false or o.run_id = $1)
     ), eligible as (
        select c.state, c.rto, c.fuel_group, c.vehicle_category,
          c.month_to_date_total - p.month_to_date_total as daily_delta,
          c.response_hash <> p.response_hash as response_changed
       from current_rows c
       join rto_registration_observations p
         on p.state = c.state and p.rto = c.rto
        and p.fuel_group = c.fuel_group and p.vehicle_category = c.vehicle_category
        and p.observation_date = c.observation_date - interval '1 day'
        and p.accepted = true and p.metric_kind = c.metric_kind
        and p.target_month = c.target_month
        and p.filters = c.filters and p.request_hash = c.request_hash
     )
     select count(*)::int as count from (
       select c.state, c.rto
       from current_rows c
       left join eligible e using (state, rto, fuel_group, vehicle_category)
       group by c.state, c.rto
        having count(*) = 6 and count(e.fuel_group) = 6
          and count(*) filter (where e.fuel_group = 'EV' and e.daily_delta <> 0 and e.response_changed) > 0
          and count(*) filter (where e.fuel_group = 'ICE' and e.daily_delta <> 0 and e.response_changed) > 0
          and count(*) filter (where e.daily_delta <> 0 and e.response_changed = false) = 0
          and sum(e.daily_delta) filter (where e.fuel_group = 'EV') <> 0
          and sum(e.daily_delta) filter (where e.fuel_group = 'ICE') <> 0
          and sum(e.daily_delta) <> 0
     ) complete`,
    [runId, date, Boolean(requireCurrentRun)],
  );
  return Number(result.rows[0]?.count ?? 0);
}

function buildDailyRegistrationReports({ period, cohort, totals, oems, generatedAt }) {
  const reports = cohort.map((member) => {
    const memberRows = totals.filter((row) => row.state === member.state && row.rto === member.rto);
    const daily = buildDailyRegistrationEvidence({ date: period.periodEnd, rows: memberRows });
    const currentFacts = memberRows.filter((row) => row.snapshotDate === period.periodEnd);
    const warnings = [];
    if (!daily.baselineEligible) warnings.push(daily.reason || DAILY_REGISTRATION_REASON);
    if (!daily.previousDayEligible) warnings.push(daily.previousDayRegistrations.reason || "Previous-day registration evidence is unavailable.");
    if (daily.status === "correction") warnings.push("A negative Daily value is preserved as a visible source correction; it is not converted to zero.");
    if (daily.oem.status !== "verified") warnings.push(daily.oem.reason);
    const trend = dateRange(addDays(period.periodEnd, -13), period.periodEnd).map((date) => {
      const point = buildDailyRegistrationEvidence({ date, rows: memberRows });
      return {
        date,
        ev: point.evRegistrations.value,
        ice: point.iceRegistrations.value,
        total: point.todayBreakdown.total,
        complete: point.baselineEligible,
        status: point.status,
        reason: point.reason,
      };
    });
    const oemSegments = buildDailyOemEvidence({ member, date: period.periodEnd, totalRows: currentFacts, oemRows: oems });
    const periodMetrics = daily.todayBreakdown;
    const previousMetrics = daily.previousDayBreakdown;
    const sourceMtd = daily.sourceMonthToDate;
    const status = daily.baselineEligible ? (warnings.length ? "ready_with_warnings" : "ready") : "needs_review";
    const correctionText = daily.status === "correction" ? " Source corrections are preserved." : "";
    const summary = daily.baselineEligible
      ? `${member.rto} recorded ${formatNumber(periodMetrics.ev)} EV and ${formatNumber(periodMetrics.ice)} ICE registrations on ${period.periodEnd}.${correctionText}`
      : `Daily registrations are unavailable for ${member.rto} on ${period.periodEnd}. ${daily.reason}`;
    return {
      state: member.state,
      rto: member.rto,
      selectionRank: member.selectionRank,
      cohortRank: null,
      previousRank: null,
      rankValue: daily.baselineEligible ? periodMetrics.ev : null,
      previousRankValue: daily.previousDayEligible ? previousMetrics.ev : null,
      status,
      currentCoverage: currentFacts.length === 6,
      summary,
      periodEv: periodMetrics.ev,
      periodIce: periodMetrics.ice,
      mtdEv: sourceMtd.ev,
      mtdIce: sourceMtd.ice,
      evShare: daily.evShare.value,
      dailyRegistration: daily,
      payload: {
        kind: "rto-daily-registration-report",
        metricKind: RTO_DAILY_METRIC_KIND,
        generatedAt: new Date(generatedAt).toISOString(),
        cadence: "daily",
        period: { start: period.periodEnd, end: period.periodEnd, label: period.periodEnd, comparisonStart: addDays(period.periodEnd, -1), comparisonEnd: addDays(period.periodEnd, -1) },
        rto: { state: member.state, name: member.rto, selectionRank: member.selectionRank, cohortRank: null, previousRank: null },
        status,
        summary,
        dailyRegistration: daily,
        metrics: {
          period: { ...periodMetrics, evShare: daily.evShare.value },
          previousPeriod: { ...previousMetrics, evShare: share(previousMetrics.ev, previousMetrics.total) },
          sourceMonthToDate: { ...sourceMtd, evShare: share(sourceMtd.ev, sourceMtd.total) },
        ...
        oems: [],
        oemEvidence: { ...daily.oem, segments: oemSegments },
        trend,
        quality: {
          currentCoverage: currentFacts.length === 6,
          registrationCoverage: daily.baselineEligible ? 6 : 0,
          comparisonEligible: daily.baselineEligible,
          previousDayEligible: daily.previousDayEligible,
          correction: daily.status === "correction",
          warnings: unique(warnings.filter(Boolean)),
          sourceFlags: currentFacts.map((row) => row.qualityFlags).filter(Boolean),
        },
        source: {
          collectedFrom: daily.freshness.collectedAt[0] ?? null,
          collectedTo: daily.freshness.collectedAt.at(-1) ?? null,
          validationContract: DAILY_REGISTRATION_SOURCE_CONTRACT,
          registrationFlowAvailable: daily.baselineEligible,
          dailyBaselineEligible: daily.baselineEligible,
          limitation: daily.reason,
          totalsTable: "rto_registration_observations.month_to_date_total",
          oemTable: daily.oem.status === "verified" ? "rto_daily_snapshots.vehicle_count" : null,
          metricKind: RTO_DAILY_METRIC_KIND,
          sourceSystem: "vahan-public-dashboard",
          metricTreatment: "Daily changes from consecutive compatible month-to-date registration observations",
          targetMonth: period.periodEnd.slice(0, 7),
          requestHashes: daily.freshness.requestHashes,
          responseHashes: daily.freshness.responseHashes,
          freshnessStatus: daily.freshness.status,
          freshnessReason: daily.freshness.reason,
          oemContract: daily.oem,
        },
      },
    };
  });
  const completeCohort = reports.length === RTO_REPORT_EXPECTED_COHORT_SIZE && reports.every((report) => Number.isFinite(report.rankValue));
  const completePrevious = reports.length === RTO_REPORT_EXPECTED_COHORT_SIZE && reports.every((report) => Number.isFinite(report.previousRankValue));
  if (completeCohort) assignRanks(reports, "rankValue", "cohortRank");
  if (completePrevious) assignRanks(reports, "previousRankValue", "previousRank");
  for (const report of reports) {
    report.payload.rto.cohortRank = report.cohortRank;
    report.payload.rto.previousRank = report.previousRank;
    const rankReason = completeCohort ? null : "Rank unavailable because daily EV changes are not valid for all 100 frozen RTOs.";
    report.dailyRegistration.rank = {
      value: report.cohortRank,
      date: period.periodEnd,
      timezone: "Asia/Kolkata",
      status: report.cohortRank ? "available" : "unavailable",
      reason: rankReason,
    };
    report.payload.dailyRegistration.rank = report.dailyRegistration.rank;
    delete report.rankValue;
    delete report.previousRankValue;
  }
  return reports;
...
function buildDailyOemEvidence({ member, date, totalRows, oemRows }) {
  const segments = [];
  for (const fuelGroup of ["EV", "ICE"]) {
    for (const vehicleCategory of ["2W", "3W", "4W"]) {
      const total = totalRows.find((row) => row.fuelGroup === fuelGroup && row.vehicleCategory === vehicleCategory);
      const oem = total?.qualityFlags?.sourceEvidence?.oem;
      if (oem?.status !== "verified") continue;
      const topFive = oemRows
        .filter((row) => row.state === member.state && row.rto === member.rto && row.snapshotDate === date
          && row.fuelGroup === fuelGroup && row.vehicleCategory === vehicleCategory)
        .sort((a, b) => Number(a.sourceRank) - Number(b.sourceRank))
        .map((row) => ({ name: row.oem, rank: row.sourceRank, count: row.vehicleCount }));
      const topFiveTotal = topFive.reduce((sum, row) => sum + Number(row.count), 0);
      if (topFiveTotal > Number(total.reportTotal)) continue;
      segments.push({ fuelGroup, vehicleCategory, targetMonth: total.targetMonth, topFive, topFiveTotal, otherUntracked: Number(total.reportTotal) - topFiveTotal });
    }
  }
  return segments;
}
...
export async function reconcileRtoReportsForRun({
  ...
  const periods = reportPeriodsForSnapshotDate(readiness.run.snapshotDate)
    .filter((period) => readiness.run.metricKind !== RTO_DAILY_METRIC_KIND || period.cadence === "daily");
  const batches = [];
...
export async function reconcileMonitoredRtoReportsForRun({ runId, snapshotDate } = {}) {
  if (!runId) throw new Error("A source run is required for monitored reports.");
  await materializeRtoReportFactsForRun({ runId, monitored: true });
  const selected = await query(`select j.config_id, j.state, j.rto from rto_daily_jobs j
    where j.run_id = $1 and not exists (select 1 from rto_daily_run_cohort_members c
      where c.run_id = j.run_id and c.config_id = j.config_id)
    order by j.state, j.rto`, [runId]);
  const periods = reportPeriodsForSnapshotDate(snapshotDate);
  let generated = 0;
  for (const member of selected.rows) {
    for (const period of periods) {
      const inputs = await loadReportInputs({ runId, period, member });
      const sourceChecksum = checksum({ version: RTO_REPORT_PAYLOAD_VERSION, period, member, totals: inputs.totalRows, oems: inputs.oemRows });
      let payload;
      if (period.cadence === "daily") {
        const report = buildRtoReportPayloads({ period, cohort: inputs.cohort, totalRows: inputs.totalRows, oemRows: inputs.oemRows })[0];
        payload = report.payload;
        payload.rto.cohortRank = null;
        payload.rto.previousRank = null;
        payload.dailyRegistration.rank = { value: null, date: period.periodEnd, timezone: "Asia/Kolkata", status: "unavailable", reason: "This RTO is outside the fixed 100-RTO ranking." };
      } else {
        payload = buildMonitoredPeriodRegistrationPayload({ period, member, totalRows: inputs.totalRows });
      }
      await query(`insert into rto_monitored_reports
        (config_id, cadence, period_start, period_end, status, source_checksum, payload)
        values ($1,$2,$3::date,$4::date,$5,$6,$7::jsonb)
        on conflict (config_id, cadence, period_start, period_end) do update set
          status = excluded.status, payload = excluded.payload,
          revision = case when rto_monitored_reports.source_checksum is distinct from excluded.source_checksum then rto_monitored_reports.revision + 1 else rto_monitored_reports.revision end,
          source_checksum = excluded.source_checksum, generated_at = now()`,
        [member.config_id, period.cadence, period.periodStart, period.periodEnd, payload.status, sourceChecksum, JSON.stringify(payload)]);
      generated++;
    }
  }
  return { monitoredRtos: selected.rows.length, reports: generated };
}

export function buildMonitoredPeriodRegistrationPayload({ period, member, totalRows = [] } = {}) {
  if (!period || !member) throw new Error("A monitored report period and RTO are required.");
  const rows = totalRows.map(normalizeTotalRow);
  if (period.cadence === "monthly") {
    const end = buildDailyRegistrationEvidence({ date: period.periodEnd, rows });
    const totals = end.sourceMonthToDate;
    const available = Number.isFinite(totals.total);
    const reason = available ? null : "Verified month-end registration totals are unavailable.";
    return { kind: "rto-period-registration-report", metricKind: RTO_DAILY_METRIC_KIND,
      cadence: "monthly", period: { start: period.periodStart, end: period.periodEnd, label: periodLabel(period) },
      rto: { state: member.state, name: member.rto, cohortRank: null, previousRank: null },
      status: available ? "ready" : "needs_review",
      summary: available ? `${member.rto} recorded ${totals.total} registrations in ${periodLabel(period)} according to the verified month-end VAHAN total.`
        : `Registrations are unavailable for ${member.rto} during ${periodLabel(period)}. ${reason}`,
      metrics: { period: { ev: available ? totals.ev : null, ice: available ? totals.ice : null, total: available ? totals.total : null } },
      trend: [], quality: { warnings: reason ? [reason] : [] },
      source: { validationContract: DAILY_REGISTRATION_SOURCE_CONTRACT, registrationFlowAvailable: available,
        metricTreatment: "Verified VAHAN month-to-date registration total at month end", limitation: reason } };
  }
  const points = dateRange(period.periodStart, period.periodEnd).map((date) => {
    const evidence = buildDailyRegistrationEvidence({ date, rows });
    return { date, status: evidence.baselineEligible ? "available" : "unavailable",
      ev: evidence.baselineEligible ? evidence.todayBreakdown.ev : null,
      ice: evidence.baselineEligible ? evidence.todayBreakdown.ice : null,
      total: evidence.baselineEligible ? evidence.todayBreakdown.total : null,
      reason: evidence.baselineEligible ? null : evidence.reason };
  });
  const complete = points.length > 0 && points.every((point) => point.status === "available");
  const sum = (key) => complete ? points.reduce((total, point) => total + Number(point[key]), 0) : null;
  const reason = complete ? null : "One or more dates lack verified consecutive registration evidence.";
  return { kind: "rto-period-registration-report", metricKind: RTO_DAILY_METRIC_KIND,
    cadence: period.cadence, period: { start: period.periodStart, end: period.periodEnd, label: periodLabel(period) },
    rto: { state: member.state, name: member.rto, cohortRank: null, previousRank: null },
    status: complete ? "ready" : "needs_review",
    summary: complete ? `${member.rto} recorded ${sum("total")} registrations during ${periodLabel(period)}.` : `Registrations are unavailable for ${member.rto} during ${periodLabel(period)}. ${reason}`,
    metrics: { period: { ev: sum("ev"), ice: sum("ice"), total: sum("total") } },
    trend: points, quality: { warnings: reason ? [reason] : [] },
    source: { validationContract: DAILY_REGISTRATION_SOURCE_CONTRACT, registrationFlowAvailable: complete, limitation: reason } };
}

export async function listMonitoredRtoReports({ userId, cadence = "daily" } = {}) {
  if (!userId) throw inputError("Sign in to see monitored RTO reports.");
  if (!RTO_REPORT_CADENCES.includes(cadence)) throw inputError("Unknown report cadence.");
  const result = await query(`select p.id as pin_id, c.state, c.rto,
    f.config_id is not null as fixed, mr.id, mr.period_end, mr.status, mr.payload->>'summary' as summary
    from rto_daily_pins p join rto_daily_snapshot_configs c on c.id = p.config_id
    left join rto_daily_fixed_cohort f on f.config_id = c.id
    left join lateral (select id, period_end, status, payload from rto_monitored_reports
      where config_id = c.id and cadence = $2 order by period_end desc limit 1) mr on true
    where p.user_id = $1 order by c.state, c.rto`, [userId, cadence]);
  return result.rows.map((row) => ({ pinId: Number(row.pin_id), state: row.state, rto: row.rto,
    fixed: row.fixed, reportId: row.id ? Number(row.id) : null, periodEnd: dateOnly(row.period_end),
    status: row.status ?? "awaiting_collection", summary: row.summary ?? null }));
}

export async function getMonitoredRtoReport({ userId, reportId } = {}) {
  if (!userId) throw inputError("Sign in to see monitored RTO reports.");
  const result = await query(`select mr.*, c.state, c.rto from rto_monitored_reports mr
    join rto_daily_snapshot_configs c on c.id = mr.config_id
    join rto_daily_pins p on p.config_id = mr.config_id and p.user_id = $1
    where mr.id = $2`, [userId, reportId]);
  const row = result.rows[0];
  if (!row) return null;
  return { id: Number(row.id), state: row.state, rto: row.rto, cadence: row.cadence,
    periodStart: dateOnly(row.period_start), periodEnd: dateOnly(row.period_end),
    status: row.status, revision: Number(row.revision), payload: row.payload };
}

export function renderMonitoredRtoReportCsv(report) {
  const metrics = report.payload?.metrics?.period ?? {};
  const rows = [
    ["cadence", "period_start", "period_end", "state", "rto", "status", "ev_registrations", "ice_registrations", "total_registrations", "reason"],
    [report.cadence, report.periodStart, report.periodEnd, report.state, report.rto, report.status,
      metrics.ev ?? "", metrics.ice ?? "", metrics.total ?? "",
      report.payload?.source?.limitation ?? ""],
  ];
  return rows.map((row) => row.map(csvCell).join(",")).join("\n") + "\n";
}

export function renderMonitoredRtoReportHtml(report) {
  const payload = report.payload ?? {};
  const metrics = payload.metrics?.period ?? {};
  const value = (number) => Number.isFinite(Number(number)) && number !== null ? formatNumber(Number(number)) : "Unavailable";
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>${escapeHtml(report.rto)} report</title>
    <style>body{font:16px Arial,sans-serif;color:#17201f;margin:36px;line-height:1.5}main{max-width:800px;margin:auto}h1{font-size:28px}table{border-collapse:collapse;width:100%}td,th{border:1px solid #ccc;padding:10px;text-align:left}th{background:#edf4f1}</style>
    </head><body><main><p>My monitored RTOs · ${escapeHtml(report.cadence)}</p><h1>${escapeHtml(report.rto)}</h1>
    <p>${escapeHtml(report.state)} · ${escapeHtml(report.periodStart)} to ${escapeHtml(report.periodEnd)}</p>
    <p><strong>${escapeHtml(statusLabel(report.status))}</strong></p><p>${escapeHtml(payload.summary ?? "")}</p>
    <table><tr><th>Registrations</th><th>Value</th></tr><tr><td>EV</td><td>${value(metrics.ev)}</td></tr>
    <tr><td>ICE</td><td>${value(metrics.ice)}</td></tr><tr><td>Total</td><td>${value(metrics.total)}</td></tr></table>
    ${payload.source?.limitation ? `<p>${escapeHtml(payload.source.limitation)}</p>` : ""}
    <p>Outside the fixed 100-RTO ranking.</p></main></body></html>`;
}
...
const batchReadSql = `select b.*, stats.verified_report_count, stats.runtime_review_count, stats.runtime_warning_count
  ...
  cross join lateral (
    select count(*) filter (where
        (b.cadence = 'daily' and payload->'source'->>'validationContract' = '${DAILY_REGISTRATION_SOURCE_CONTRACT}')
        or (b.cadence <> 'daily' and payload->'source'->>'validationContract' = 'public-stock-v2'))::int as verified_report_count,
      count(*) filter (where
        (b.cadence = 'daily' and payload->'source'->>'validationContract' is distinct from '${DAILY_REGISTRATION_SOURCE_CONTRACT}')
        or (b.cadence <> 'daily' and payload->'source'->>'validationContract' is distinct from 'public-stock-v2')
        or status = 'needs_review')::int as runtime_review_count,
      count(*) filter (where status = 'ready_with_warnings' and (
        (b.cadence = 'daily' and payload->'source'->>'validationContract' = '${DAILY_REGISTRATION_SOURCE_CONTRACT}')
        or (b.cadence <> 'daily' and payload->'source'->>'validationContract' = 'public-stock-v2')))::int as runtime_warning_count
...
export async function listRtoReportsForBatch(batchId, {
  ...
  if (status) {
    values.push(status);
    clauses.push(`(case when
      (b.cadence = 'daily' and rr.payload->'source'->>'validationContract' = '${DAILY_REGISTRATION_SOURCE_CONTRACT}')
      or (b.cadence <> 'daily' and rr.payload->'source'->>'validationContract' = 'public-stock-v2')
      then rr.status else 'needs_review' end) = $${values.length}`);
  }
  values.push(boundedInt(limit, 1, 500, 100));
...
export async function rtoReportReadinessForDate(snapshotDate) {
  const date = dateOnly(snapshotDate);
  ...
  const result = await query(
    `
      ...
      limit 1
    `,
    [date, RTO_DAILY_METRIC_KIND],
  );
  if (!result.rows[0]) return null;
  return getRtoReportReadiness({ runId: Number(result.rows[0].id) });
}
...
export function renderRtoReportHtml(report) {
  ...
  if (payload.cadence === "daily") {
    ...
    const available = daily.baselineEligible === true;
    return `<!doctype html><html lang="en"><meta charset="utf-8"><title>${escapeHtml(payload.rto.name)} Daily Report</title>
      ...
      ${report?.insightSummary ? `<h2>What changed today?</h2><p>${escapeHtml(report.insightSummary.text)}</p><small>${report.insightSummary.source === "groq" ? "AI-assisted wording from verified VAHAN figures" : "Summary from verified VAHAN figures"}</small>` : ""}
      <h2>${available ? "Daily registrations" : "Daily registrations unavailable"}</h2><p>${escapeHtml(daily.reason ?? "Calculated from consecutive compatible Public Dashboard monthly-registration observations.")}</p>
      <table><thead><tr><th>Metric</th><th>Date (IST)</th><th>Value</th><th>Reason</th></tr></thead><tbody>
      ${fields.map(([label,field])=>`<tr><td>${label}</td><td>${escapeHtml(field.date)}</td><td>${Number.isFinite(field.value) ? escapeHtml(formatNumber(field.value)) : "Unavailable"}</td><td>${escapeHtml(field.reason ?? field.status)}</td></tr>`).join("")}</tbody></table>
      <h2>Source month-to-date registrations</h2><p>EV ${escapeHtml(formatNumber(daily.sourceMonthToDate?.ev))}; ICE ${escapeHtml(formatNumber(daily.sourceMonthToDate?.ice))}; total ${escapeHtml(formatNumber(daily.sourceMonthToDate?.total))}.</p>
...
export async function pruneRtoReportingData({
  ...
  const monitoredDailyDelete = await query(
    "delete from rto_monitored_reports where cadence = 'daily' and period_end < $1::date",
    [dailyCutoff],
  );
  ...
  return {
    ...
    deletedMonitoredDailyReports: monitoredDailyDelete.rowCount,
    deletedTotalFacts: totalDelete.rowCount,
    deletedOemFacts: oemDelete.rowCount,
  };
}

async function loadReportInputs({ runId, period, member = null }) {
  ...
  const scopeSql = member
    ? "state = $3 and rto = $4"
    : `exists (select 1 from rto_daily_run_cohort_members c where c.run_id = $3
        and c.state = rto_daily_report_totals.state and c.rto = rto_daily_report_totals.rto)`;
  const oemScopeSql = member
    ? "state = $3 and rto = $4"
    : `exists (select 1 from rto_daily_run_cohort_members c where c.run_id = $3
        and c.state = rto_daily_oem_totals.state and c.rto = rto_daily_oem_totals.rto)`;
  const scopeParams = member ? [member.state, member.rto] : [runId];
  const [cohortResult, totalResult, oemResult] = await Promise.all([
    member
      ? Promise.resolve({ rows: [{ state: member.state, rto: member.rto, cohort_rank: 1 }] })
      : query(`select state, rto, cohort_rank from rto_daily_run_cohort_members
        where run_id = $1 order by cohort_rank, state, rto`, [runId]),
    query(
      `
        select
          snapshot_date,
        ...
        where snapshot_date between $1::date and $2::date
          and exists (select 1 from rto_daily_scrape_reports r
            where r.id = rto_daily_report_totals.source_report_id and ${verifiedRtoRegistrationSourceSql("r")})
          and metric_kind = '${RTO_DAILY_METRIC_KIND}'
          and ${scopeSql}
        order by state, rto, snapshot_date, fuel_group, vehicle_category
      `,
      [loadStart, period.periodEnd, ...scopeParams],
    ),
    query(
      `
        select
  ...
  const result = await query(
    `
      ...
      where exists (
        ...
        and r.snapshot_date <= (now() at time zone 'Asia/Kolkata')::date
...
function reportCsvRows(report) {
  ...
  if ((report.cadence ?? payload.cadence) === "daily") {
    const daily = payload.dailyRegistration;
    const metricRows = ["previousDayRegistrations","evRegistrations","iceRegistrations","evShare","rank"].map(name => [
        "daily",daily[name].date,daily.timezone,report.state ?? payload.rto?.state,report.rto ?? payload.rto?.name,
        name,daily[name].value,daily[name].status,daily[name].reason,
      ]);
    for (const name of ["ev", "ice", "total"]) metricRows.push([
      "daily", daily.date, daily.timezone, report.state ?? payload.rto?.state, report.rto ?? payload.rto?.name,
      `sourceMonthToDate.${name}`, daily.sourceMonthToDate?.[name] ?? null, daily.sourceMonthToDate?.[name] === null ? "unavailable" : "source_observation", null,
    ]);
    return [["cadence","date","timezone","state","rto","metric","value","status","reason"], ...metricRows];
  }
  const header = [
    "cadence",
    "period_start",

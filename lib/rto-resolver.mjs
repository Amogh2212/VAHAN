...
function scoreEntry(entry, query) {
  const aliases = uniqueNormalized([entry.normalized, ...(entry.aliases ?? [])]);
  if (aliases.some((alias) => alias === query)) return { score: 100, method: "exact" };
  ...
  if (aliases.some((alias) => query.includes(alias) && alias.length >= 4)) return { score: 78, method: "query-substring" };

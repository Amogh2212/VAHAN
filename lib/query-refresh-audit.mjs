...
export function publicDashboardRefreshEligibility(filters = {}) {
  ...
  const selectedCategories = new Set((filters.vehicleCategories ?? []).map((label) => String(label).trim().toUpperCase()));
  const excludedCategories = (filters.excludedVehicleCategories ?? []).map((label) => String(label).trim().toUpperCase());
  const isFourWheelerPair = selectedCategories.has("LIGHT MOTOR VEHICLE") && selectedCategories.has("LIGHT PASSENGER VEHICLE");
  const onlyImplicitFourWheelerExclusion = isFourWheelerPair
    && excludedCategories.length === 1
    && excludedCategories[0] === "FOUR WHEELER (INVALID CARRIAGE)";
  if (
    filters.excludedVehicleGroups?.length
    || filters.excludedNorms?.length
    || filters.excludedVehicleClasses?.length
    || (excludedCategories.length && !onlyImplicitFourWheelerExclusion)
  ) {
  ...
  // The Public Dashboard exposes vehicle sub-categories, classes, emissions,
  // and fuels as inclusive multi-select controls.  The scraper serializes
  // categories/classes/norms as the dashboard's array parameters and collects
  // every selected fuel independently so each saved row retains its fuel
  // identity.  Do not reintroduce a one-value gate for these controls.
...
export async function createQueryRefreshAudit({
  ...
  const result = await query(
    ...
    [
      canonicalKey,
      canonicalRefreshJson(filters),
      JSON.stringify(requestedMonths ?? []),
  ...
  await query(
    `
      ...
      set outcome = coalesce($2, outcome),
          refresh_job_id = coalesce($3, refresh_job_id),
          error_message = $4,
          coverage_json = coalesce($5::jsonb, coverage_json),
          updated_at = now()
      where id = $1
    `,

...
export const PUBLIC_REGISTRATION_SCOPE_CONTRACT = "public-registration-mtd-v1";
...
function decode(value) {
  const entities = { "&amp;": "&", "&quot;": '"', "&#39;": "'", "&apos;": "'", "&lt;": "<", "&gt;": ">" };
  // Decode exactly once; an encoded entity must not become a second entity.
  return value.replace(/&(?:amp|quot|apos|lt|gt|#39);/g, (entity) => entities[entity]);
}
...
export function publicSelectOptions(html, id) {
  ...
  return [...select[2].matchAll(/<option\b([^>]*)>([\s\S]*?)<\/option>/gi)]
    .map((match) => {
      // Catalog labels are plain text. Reject unexpected markup, do not try to
      // sanitize malformed HTML into a seemingly valid office or state name.
      if (/[<>]/.test(match[2])) throw new Error("Public Dashboard option label contains unexpected markup.");
      return { value: decode(attribute(match[1], "value") ?? ""), label: decode(match[2].trim()) };
    });
}
...
export function verifiedRtoSourceSql(alias = "r") {
  ...
  return `(${v}->>'contract' = '${PUBLIC_SCOPE_CONTRACT}'
    ...
    and (${alias}.scraped_at at time zone 'Asia/Kolkata')::date = ${alias}.snapshot_date
    and ${alias}.target_month = to_char(${alias}.snapshot_date, 'YYYY-MM')
...
export function verifiedRtoRegistrationSourceSql(alias = "r") {
  if (!/^[a-z]+$/.test(alias)) throw new Error("Invalid SQL alias.");
  const v = `${alias}.evidence->'validation'`;
  return `(${v}->>'contract' = '${PUBLIC_REGISTRATION_SCOPE_CONTRACT}'
    and ${v}->'mapping'->>'state' = ${alias}.state
    and ${v}->'mapping'->>'rto' = ${alias}.rto
    and ${v}->'mapping'->>'stateCode' = ${alias}.evidence->'filters'->>'stateCode'
    and ${v}->'mapping'->>'rtoCode' = ${alias}.evidence->'filters'->>'rtoCode'
    and ${v}->'monthToDateTotal' = to_jsonb(${alias}.report_total)
    and ${v}->>'requestHash' ~ '^[a-f0-9]{64}$'
    and ${v}->>'responseHash' ~ '^[a-f0-9]{64}$'
    and ${v}->'mapping'->>'catalogHash' ~ '^[a-f0-9]{64}$'
    and ${v}->>'timePeriod' = '0'
    and ${v}->>'calendarType' = '3'
    and ${v}->>'targetMonth' = ${alias}.target_month
    and ${alias}.source = 'vahan-public-dashboard'
    and ${alias}.metric_kind = 'registration_month_to_date'
    and (${alias}.scraped_at at time zone 'Asia/Kolkata')::date = ${alias}.snapshot_date
    and ${alias}.target_month = to_char(${alias}.snapshot_date, 'YYYY-MM')
    and ${alias}.evidence->'filters'->>'archiveScope' = 'ACTIVE_ONLY'
    and ${alias}.fuel_group in ('EV', 'ICE')
    and ${alias}.vehicle_category in ('2W', '3W', '4W')
    and ${alias}.source_row_count between 0 and 5
    and (( ${alias}.evidence->'oem'->>'status' = 'unavailable'
      and nullif(${alias}.evidence->'oem'->>'reason', '') is not null
      and ${alias}.source_row_count = 0)
      or (${alias}.evidence->'oem'->>'status' = 'verified'
        and ${alias}.evidence->'oem'->'validation'->>'contract' = '${PUBLIC_REGISTRATION_SCOPE_CONTRACT}'
        and ${alias}.evidence->'oem'->'validation'->>'targetMonth' = ${alias}.target_month
        and ${alias}.evidence->'oem'->'validation'->>'calendarType' = '3'
        and ${alias}.evidence->'oem'->'validation'->>'timePeriod' = '0'
        and ${alias}.evidence->'oem'->'validation'->'filters' = ${alias}.evidence->'filters'
        and ${alias}.evidence->'oem'->'validation'->>'requestUrl' like 'https://analytics.parivahan.gov.in/analytics/publicdashboard/%'
        and ${alias}.evidence->'oem'->'validation'->>'requestHash' ~ '^[a-f0-9]{64}$'
        and ${alias}.evidence->'oem'->'validation'->>'responseHash' ~ '^[a-f0-9]{64}$'
        and ${alias}.evidence->'oem'->'topFiveTotal' = (
          select to_jsonb(coalesce(sum(s.vehicle_count), 0)::int)
          from rto_daily_snapshots s
          where s.report_id = ${alias}.id and s.metric_kind = 'registration_month_to_date')
        and ${alias}.evidence->'oem'->'otherUntracked' = to_jsonb(${alias}.report_total - (${alias}.evidence->'oem'->>'topFiveTotal')::int)
        and ${alias}.evidence->'topMakerRows' = (
          select coalesce(jsonb_agg(jsonb_build_object('maker', s.oem, 'vehicle_count', s.vehicle_count, 'rank', s.source_rank) order by s.source_rank), '[]'::jsonb)
          from rto_daily_snapshots s
          where s.report_id = ${alias}.id and s.metric_kind = 'registration_month_to_date')
        and ${alias}.source_row_count = (
          select count(*)::int from rto_daily_snapshots s
          where s.report_id = ${alias}.id and s.metric_kind = 'registration_month_to_date')))
    and (${alias}.report_total > 0 or ${v}->'zeroConfirmed' = 'true'::jsonb))`;
}
...
export function validatePublicRegistrationEvidence(report) {
  const v = report.evidence?.validation;
  const filters = report.evidence?.filters;
  const oem = report.evidence?.oem;
  const hex = (value) => typeof value === "string" && /^[a-f0-9]{64}$/.test(value);
  const validOemStatus = oem?.status === "verified" || (oem?.status === "unavailable" && Boolean(oem.reason));
  const makerTotal = report.rows?.reduce((sum, row) => sum + Number(row.vehicle_count ?? row.vehicleCount), 0);
  const makerValidation = oem?.validation;
  const makerVerified = oem?.status !== "verified" || (
    makerValidation?.contract === PUBLIC_REGISTRATION_SCOPE_CONTRACT
    && makerValidation.targetMonth === v?.targetMonth
    && makerValidation.calendarType === "3"
    && makerValidation.timePeriod === "0"
    && JSON.stringify(makerValidation.filters) === JSON.stringify(filters)
    && monthlyMakerRequestMatchesHeadline(makerValidation.requestUrl, filters)
    && hex(makerValidation.requestHash)
    && hex(makerValidation.responseHash)
    && Number.isFinite(Date.parse(makerValidation.observedAt))
    && oem.topFiveTotal === makerTotal
    && oem.otherUntracked === report.reportTotal - makerTotal
    && report.evidence.topFiveTotal === makerTotal
    && JSON.stringify(report.evidence.topMakerRows) === JSON.stringify(report.rows)
  );
  if (v?.contract !== PUBLIC_REGISTRATION_SCOPE_CONTRACT
    || v.mapping?.state !== report.state || v.mapping?.rto !== report.rto
    || !hex(v.mapping?.catalogHash) || !hex(v.requestHash) || !hex(v.responseHash)
    || !Number.isFinite(Date.parse(v.observedAt)) || v.monthToDateTotal !== report.reportTotal
    || v.mapping.stateCode !== filters?.stateCode || v.mapping.rtoCode !== filters?.rtoCode
    || !filters?.vehicleCategories?.length || !filters?.fuels?.length
    || filters?.archiveScope !== "ACTIVE_ONLY" || filters?.targetMonth !== v.targetMonth
    || v.timePeriod !== "0" || v.calendarType !== "3"
    || report.metricKind !== "registration_month_to_date" || report.source !== "vahan-public-dashboard"
    || !validOemStatus || !makerVerified || (oem.status === "unavailable" && report.rows?.length)
    || (oem.status === "verified" && v.sourceRowCount !== report.rows?.length)
    || (report.reportTotal === 0 && v.zeroConfirmed !== true)) {
    throw new Error("Invalid VAHAN report: monthly registration source evidence is unverified.");
  }
}

function monthlyMakerRequestMatchesHeadline(requestUrl, filters) {
  let url;
  try { url = new URL(requestUrl); } catch { return false; }
  if (url.origin !== "https://analytics.parivahan.gov.in"
    || !url.pathname.startsWith("/analytics/publicdashboard/")) return false;
  const q = url.searchParams;
  const sameList = (name, values = []) => JSON.stringify(q.getAll(name).sort()) === JSON.stringify([...values].sort());
  const year = String(filters?.targetMonth ?? "").slice(0, 4);
  return q.get("targetMonth") === filters?.targetMonth
    && q.get("calendarType") === "3" && q.get("timePeriod") === "0"
    && q.get("fromYear") === year && q.get("toYear") === year
    && q.get("stateCode") === filters?.stateCode && q.get("rtoCode") === filters?.rtoCode
    && sameList("vehicleSubCategories[]", filters?.vehicleCategories)
    && sameList("vehicleClasses[]", filters?.vehicleClasses)
    && sameList("vehicleFuels[]", filters?.fuels)
    && sameList("vehicleStatus[]", [])
    && q.get("archiveTypeAC") === "ACTIVE_COMPLIANT"
    && q.get("archiveTypeANC") === "ACTIVE_NON_COMPLIANT"
    && !q.get("archiveTypePA") && !q.get("archiveTypeTA") && !q.get("archiveTypeNA");
}
...
// Registration and stock requests use the same exact option catalogs. The
// metric contract is determined by the request's timePeriod and endpoint, not
// by a different state/RTO/category resolver.
export const resolvePublicRegistrationScope = resolvePublicStockScope;

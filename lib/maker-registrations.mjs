...
export async function upsertMakerRegistrationRows(rows, { batchSize = 500 } = {}) {
  ...
  const normalizedRows = dedupeMakerRegistrationRows(rows.map(normalizeMakerRegistrationRow).filter(isValidMakerRegistrationRow));
  let count = 0;
...
function dedupeMakerRegistrationRows(rows) {
  const byKey = new Map();
  for (const row of rows) {
    byKey.set(makerRegistrationKey(row), row);
  }
  return [...byKey.values()];
}

function makerRegistrationKey(row) {
  return [
    row.year,
    row.month,
    row.state,

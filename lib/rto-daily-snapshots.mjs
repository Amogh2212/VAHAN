...
import { validatePublicStockEvidence } from "./public-dashboard-scope.mjs";

export const RTO_DAILY_FUEL_GROUPS = ["EV", "ICE"];
export const RTO_DAILY_CATEGORIES = ["2W", "3W", "4W"];
export const RTO_DAILY_METRIC_KIND = "active_stock";
...
export function buildRankedStockSnapshotRows({ sourceRows, state, rto, snapshotDate, targetMonth, fuelGroup, vehicleCategory, metadata = {} }) {
...
export function validateRtoDailyReport(report, expected = {}) {
  ...
  validatePublicStockEvidence(report);
...
export function validateStockJobPersistence({ job, reports, rows, now = new Date() }) {
  if (job.snapshotDate !== snapshotDateKey(now) || job.targetMonth !== job.snapshotDate.slice(0, 7)) {
    throw new Error("Active stock can only be collected for today's IST observation date and month; historical backfill is unsupported.");
  }
  for (const report of reports) {
    if (snapshotDateKey(new Date(report.scrapedAt)) !== job.snapshotDate) {
      throw new Error("Stock collection crossed the IST date boundary; discard this attempt and collect a new current-day job.");
    }
  }
  ...
  const actual = rows.map(row => {
    if (row.state !== job.state || row.rto !== job.rto || row.snapshotDate !== job.snapshotDate
      ...
      throw new Error("OEM row does not match the claimed stock job scope/date/source.");
    }
...
export async function completeRtoDailyJob({ job, workerId, reports = [], rows = [] } = {}) {
  ...
  if (new Set(reports.map(report => `${report.fuelGroup}|${report.vehicleCategory}`)).size !== 6) {
    throw new Error("Expected six distinct fuel/category reports.");
  }
  validateStockJobPersistence({ job, reports, rows });
  ...
  return transaction(async (tx) => {
    ...
    // Replace this claimed observation atomically: makers can leave the top five
    // on a retry. Retaining old members would corrupt the new headline evidence.
    await tx(`delete from rto_daily_snapshots where snapshot_date = $1::date
      and target_month = $2 and state = $3 and rto = $4 and metric_kind = $5`,
    [job.snapshotDate, job.targetMonth, job.state, job.rto, RTO_DAILY_METRIC_KIND]);
...
async function ensureCycleWithQuery(runQuery, { snapshotDate, targetMonth, workerCount = 2, state = null } = {}) {
  const result = await runQuery(
    `
      ...
      values ('running', $1::date, $2, $3, $4::jsonb, 'active_stock', 'vahan-public-dashboard')

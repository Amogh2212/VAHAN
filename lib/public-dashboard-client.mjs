...
const STATE_CODES = new Map([
  ...
  ["telangana", "TS"], ["tripura", "TR"], ["ut of dnh and dd", "DN"],
  ["dadra and nagar haveli and daman and diu", "DN"], ["uttar pradesh", "UP"],
...
export async function fetchPublicDashboardRows({
  ...
  for (const fuel of normalizedFuels) {
    ...
    // The official table omits an explicit fuel when its count is zero.  A
    // zero row preserves the requested filter context and stops a valid zero
    // from being reported as a missing data month.
    for (const month of monthsToStore) {
      const vehicle_count = counts.get(month) ?? 0;
      rows.push({

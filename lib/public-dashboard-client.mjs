...
function publicStockParams({ state, rto, vehicleCategories = [], vehicleClasses = [], fuels = [] }) {
  return {
    fromYear: "",
    toYear: "",
    stateCode: publicStateCode(state, rto),
...
export function parsePublicTopMakers(response) {
  ...
  const makers = labels.map((label, index) => {
    ...
    const count = numberOrNull(values[index]);
...
export async function fetchPublicRtoStockSegment({
  ...
  const params = publicStockParams({ state, rto, vehicleCategories, vehicleClasses, fuels });

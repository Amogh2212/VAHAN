...
import {
  resolvePublicRegistrationScope,
  resolvePublicStockScope,
  PUBLIC_REGISTRATION_SCOPE_CONTRACT,
  PUBLIC_SCOPE_CONTRACT,
} from "./public-dashboard-scope.mjs";
...
const ALL = "ALL";
export const PUBLIC_MONTHLY_MAKER_UNAVAILABLE_REASON =
  "The Public Dashboard Top 5 Makers request does not expose the monthly table's calendarType or a target-month selector, so its maker totals cannot be proven to match this monthly-registration observation.";
...
export function parsePublicMonthlyCounts(response, year) {
  ...
  for (const item of response) {
    ...
    if (counts.has(period.month)) {
      throw new Error(`Public Dashboard returned duplicate monthly registration rows for ${period.year}-${String(period.month).padStart(2, "0")}.`);
    }
...
export function reconcileMonthlyTopMakers({ headlineTotal, makers = [] } = {}) {
  if (!Number.isSafeInteger(headlineTotal) || headlineTotal < 0) {
    throw new Error("Monthly registration headline total is invalid.");
  }
  if (!Array.isArray(makers) || makers.length > 5) {
    throw new Error("Monthly registration maker evidence must contain at most five rows.");
  }
  const seen = new Set();
  let topFiveTotal = 0;
  for (const [index, row] of makers.entries()) {
    const maker = compact(row?.maker);
    const count = Number(row?.count);
    const rank = Number(row?.rank);
    if (!maker || seen.has(maker.toUpperCase()) || !Number.isSafeInteger(count) || count < 0 || rank !== index + 1) {
      throw new Error("Monthly registration maker evidence is invalid.");
    }
    seen.add(maker.toUpperCase());
    topFiveTotal += count;
  }
  if (topFiveTotal > headlineTotal) {
    throw new Error("Monthly registration top-five OEM total exceeds the matching headline total.");
  }
  return { topFiveTotal, otherUntracked: headlineTotal - topFiveTotal };
}
...
export async function fetchPublicRtoRegistrationSegment({
  state,
  rto,
  targetMonth,
  vehicleCategories = [],
  ...
  if (!/^\d{4}-\d{2}$/.test(String(targetMonth ?? ""))) {
    throw new Error("Public Dashboard registration requests require targetMonth in YYYY-MM format.");
  }
  if (!vehicleCategories.length || !fuels.length) {
    throw new Error("Public Dashboard registration requests require explicit category and fuel filters.");
  }
  const year = Number(String(targetMonth).slice(0, 4));
  const month = Number(String(targetMonth).slice(5, 7));
  if (!Number.isInteger(year) || month < 1 || month > 12) {
    throw new Error("Public Dashboard registration targetMonth is invalid.");
  }
  const scope = await resolvePublicRegistrationScope({
    state, rto, vehicleCategories, vehicleClasses, fuels, fetchImpl, timeoutMs, beforeRequest,
  });
  const params = {
    ...publicParams({
      state,
    ...
    rtoCode: scope.rtoCode,
    vehicleFuels: scope.fuels,
    calendarType: "3",
    timePeriod: "0",
  };
  const query = publicMonthlyQueryString(params);
  await beforeRequest();
  const response = await publicJson(MONTHLY_TABLE_PATH, query, "monthly registration", { fetchImpl, timeoutMs });
  const counts = parsePublicMonthlyCounts(response, year);
  if (!counts.has(month)) {
    throw new Error(`Public Dashboard returned no explicit monthly registration value for ${targetMonth}; missing evidence is not zero.`);
  }
  const total = counts.get(month);
  if (!Number.isSafeInteger(total) || total < 0) {
    throw new Error("Public Dashboard returned an invalid monthly registration total.");
  }
  const observedAt = new Date().toISOString();
  const requestHash = createHash("sha256").update(`${MONTHLY_TABLE_PATH}?${query}`).digest("hex");
  ...
  return {
    ...
    validation: {
      contract: PUBLIC_REGISTRATION_SCOPE_CONTRACT,
    ...
    freshness: {
      ...
      status: "unconfirmed",
      reason: "The endpoint exposes the fetched monthly value but no upstream refresh timestamp; identical cross-day responses are therefore not treated as a verified zero change.",
    },
    oem: {
      status: "unavailable",
      reason: PUBLIC_MONTHLY_MAKER_UNAVAILABLE_REASON,
      makers: [],
      topFiveTotal: null,
      otherUntracked: null,
    },

...
export async function queryRegistrationRows(filters = {}) {
...
function buildRegistrationWhere(filters, { includeDate, includeFuel }) {
  const values = [];
  const clauses = [];
  const add = (sql, value) => {
    values.push(value);
  ...
  for (const [column, key] of FILTER_CONTEXT_FIELDS) {
    add(`coalesce(${column}, 'ALL') = ?`, contextValue(filters[key]));
  }

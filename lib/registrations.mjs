...
function buildRegistrationWhere(filters, { includeDate, includeFuel, stateRtoMode = "aggregate" }) {
  ...
  const contextFilters = {
    ...filters,
    // The semantic query parser stores an exact fuel selection in
    // selectedFuelTypes, while persisted registration rows use the
    // fuel_filter context column. Keep both query shapes equivalent.
    fuelFilters: filters.fuelFilters?.length
      ? filters.fuelFilters
      : filters.selectedFuelTypes ?? [],
  };
  for (const [column, key] of FILTER_CONTEXT_FIELDS) {
    if (column === "fuel_filter" && savedFuelContextCandidates(filters).length > 1) {
      addIn(`coalesce(${column}, 'ALL')`, savedFuelContextCandidates(filters));
    } else {
...
export function savedFuelContextCandidates(filters = {}) {
  const selected = filters.fuelFilters?.length ? filters.fuelFilters : filters.selectedFuelTypes ?? [];
  const preferred = contextValue(selected);
  // The Public Dashboard can save an exact B.O.V. fuel row under a two-fuel
  // checkbox selection. The row's fuel_type still identifies B.O.V. exactly.
  if (!filters.fuelFilters?.length && contextValue(filters.selectedFuelTypes) === "ELECTRIC(BOV)") {
    return [preferred, contextValue(["ELECTRIC(BOV)", "PURE EV"])];
  }
  return [preferred];
}

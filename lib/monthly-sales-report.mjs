...
export function buildMonthlySalesReport({
  ...
  const evCount = (items) => sumRows(items.filter((row) => evIceFuelGroup(row) === "EV"));
  ...
  return {
    ...
    insightFacts: {
      ...
      total,
      previousTotal,
      evShare: share(evCount(allCurrentRows), sumRows(allCurrentRows)),
      previousEvShare: share(evCount(allPreviousRows), sumRows(allPreviousRows)),

...
export function buildMonthlySalesReport({
  ...
  const allPreviousRows = reportBaseRows.filter((row) => rowMonth(row) === previousMonth);
  ...
  const comparableMonths = completeTrendMonths.has(reportMonth) && completeTrendMonths.has(previousMonth);
  const verifiedEvShare = (items) => {
    if (!items.length || items.some((row) => normalizeLabel(row.fuel_type) === "ALL")) return null;
    const evRows = items.filter((row) => evIceFuelGroup(row) === "EV");
    const iceRows = items.filter((row) => evIceFuelGroup(row) === "ICE");
    if (!evRows.length || !iceRows.length) return null;
    return share(sumRows(evRows), sumRows(items));
  };
  const categoryChanges = comparableMonths ? categorySales
    .filter((item) => item.status === "available" && item.previousRowCount > 0 && item.delta !== 0)
    .filter((item) => reportScope.type !== "all" || (
      (item.currentHasNationalAggregate && item.previousHasNationalAggregate) ||
      (item.stateCount >= expectedStateCount && item.previousStateCount >= expectedStateCount)
    ))
  ...
  return {
    ...
    insightFacts: {
      comparableMonths,
      currentComplete: completeTrendMonths.has(reportMonth),
      previousComplete: completeTrendMonths.has(previousMonth),
      total,
      previousTotal,
      evShare: verifiedEvShare(allCurrentRows),
      previousEvShare: verifiedEvShare(allPreviousRows),
      categoryChange: categoryChanges[0] ? {
        title: categoryChanges[0].title,
        current: categoryChanges[0].currentCount,
        previous: categoryChanges[0].previousCount,
        delta: categoryChanges[0].delta,
      } : null,
    },
    sections: [
      {
        ...
        metrics: {
          total,
          previousTotal: comparableMonths ? previousTotal : null,
          delta: comparableMonths ? total - previousTotal : null,
          percentChange: comparableMonths ? percentChange(total, previousTotal) : null,
          marketShare: share(total, sumRows(allCurrentRows)),
          states: loadedStateCount,
          expectedStates: expectedStateCount,
        },
        narrative: overviewNarrative(scopeLabel, reportMonth, total, previousTotal, loadedStateCount, expectedStateCount, reportScope, comparableMonths, completeTrendMonths.has(reportMonth)),
      },
      {
        id: "fuel_mix",
...
export function renderMonthlySalesReportHtml(report, { includeShell = true } = {}) {
  ...
  const body = `
    <main class="report-document">
      ...
      ${report.insightSummary ? `<section class="report-section report-insight"><h2>What changed this month?</h2><p>${escapeHtml(report.insightSummary.text)}</p><small>${report.insightSummary.source === "groq" ? "AI-assisted wording from verified VAHAN figures" : "Summary from verified VAHAN figures"}</small></section>` : ""}
      ${sections}
  ...
  if (section.chartType === "metric") {
    ...
    return `
      <div class="report-metric-grid">
        ...
        <article><span>Previous month</span><strong>${metrics.previousTotal === null ? "Unavailable" : formatNumber(metrics.previousTotal)}</strong></article>
        <article><span>Change</span><strong>${metrics.delta === null ? "Unavailable" : formatDelta(metrics.delta)}</strong></article>
          ...
          <table>
            ...
            <tbody>${(group.brands ?? []).map((brand) => `
              <tr>
                ...
                <td>${brand.matchedMakers?.length ? formatNumber(brand.count) : "—"}</td>
...
function segmentSummary(rows, segment, fuelSelection, currentMonth, previousMonth, months, { preferNationalAggregate = false, locationScope = null } = {}) {
  ...
  return {
    ...
    previousStateCount: uniqueSorted(previousRows.map((row) => row.state)).length,
    currentHasNationalAggregate: currentRows.some(isNationalAggregateRow),
    previousHasNationalAggregate: previousRows.some(isNationalAggregateRow),
...
function overviewNarrative(scopeLabel, month, total, previousTotal, states, expectedStates = states, locationScope = null, comparable = true, currentComplete = true) {
  const reportScope = normalizeLocationScope(locationScope);
  if (!currentComplete) return `${scopeLabel} source coverage for ${displayMonth(month)} is incomplete. The saved ${formatNumber(total)} registrations are partial and cannot support a month-on-month comparison.`;
  if (!comparable) return `${scopeLabel} registrations in ${displayMonth(month)} were ${formatNumber(total)}. A month-on-month comparison is unavailable because previous-month source coverage is incomplete.`;

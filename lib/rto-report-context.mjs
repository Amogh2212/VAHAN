...
export async function loadRtoReportWithOptionalFactorContext({
  ...
  if ((report.cadence ?? report.payload?.cadence) === "daily" && report.payload?.source?.registrationFlowAvailable !== true) {
    return { ...report, explanations: [], factorContext: { status: "unavailable",
      message: "Daily registrations are unavailable; explanations based on stock observations are withheld." } };
  }

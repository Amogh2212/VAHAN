...
function compareObservationSets(currentRows, previousRows, currentDate, previousDate) {
  ...
  const changedByFuel = new Map(fuelGroups.map((fuel) => [fuel, false]));
  for (const row of currentRows) {
    ...
    const delta = Number(row.reportTotal) - Number(prior.reportTotal);
    if (delta !== 0 && currentEvidence.validation.responseHash === priorEvidence.validation.responseHash) {
      return { eligible: false, reason: "A registration total changed without a changed source response; the comparison cannot be verified.", values: new Map() };
    }
    if (delta !== 0) changedByFuel.set(row.fuelGroup, true);
    values.set(scopeKey(row), delta);
  }
  if (fuelGroups.some((fuel) => !changedByFuel.get(fuel))) {
    return { eligible: false, reason: "An EV or ICE month-to-date total is unchanged without a source refresh timestamp; its zero Daily change is unverified.", values: new Map() };
  }
  const totals = totalsFromValues(values);
  if (totals.ev === 0 || totals.ice === 0 || totals.total === 0) {
    return { eligible: false, reason: "The combined registration total is unchanged without a source refresh timestamp; zero Daily change is unverified.", values: new Map() };
  }
